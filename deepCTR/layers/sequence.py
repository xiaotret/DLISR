...
class SequencePoolingLayer(Layer):
    ...
    def call(self, seq_value_len_list, mask=None, **kwargs):
        if self.supports_masking: # 支持mask的话，输入就改明确哪些是需要mask掉的？
            ...
            user_behavior_length = tf.reduce_sum(mask, axis=-1, keep_dims=True) # (batch_size, 1) 得到变长值
            mask = tf.expand_dims(mask, axis=2) # (batch_size,seq_len_max,1) ?
        else:
            uiseq_embed_list, user_behavior_length = seq_value_len_list # [seq_value,seq_len]

            mask = tf.sequence_mask(user_behavior_length,
                                    self.seq_len_max, dtype=tf.float32) # user_behavior_length：(batch_size, 1) 补充到最大长度得到mask (batch_size, 1,seq_len_max)
            mask = tf.transpose(mask, (0, 2, 1)) # (batch_size,seq_len_max,1)

        embedding_size = uiseq_embed_list.shape[-1]

        mask = tf.tile(mask, [1, 1, embedding_size]) # 复制最后一维用于乘，类似广播机制  # (batch_size,seq_len_max,embedding_size)
        ...
        if self.mode == "mean":
            hist = tf.div(hist, user_behavior_length + self.eps) # 除以变长输入！注意eps

        hist = tf.expand_dims(hist, axis=1) # (batch_size, 1, embedding_size)
        return hist

    def compute_output_shape(self, input_shape):
        if self.supports_masking: # 因为call的输入不同
