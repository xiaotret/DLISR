...
def conv2d_bn(x, filters, num_row, num_col, padding='same', strides=(1, 1), name=None):
    ...
    x = tf.keras.layers.BatchNormalization(axis=channel_axis, scale=False, name=bn_name)(x)
    x = Activation('relu', name=name)(x)
    return x


def inception_layer(x, EMBEDDING_DIM, filterNums=(64,96,64,96,64),pooling='global_average'):
    """
    ...
    if pooling == 'global_avg':
        ...
        x = MaxPooling2D(pool_size=(2,1))(x) # filter num* len
        x = tf.keras.layers.Flatten()(x) # 1D
    elif pooling == 'none':  # FC 提取特征
        x = tf.keras.layers.Flatten () (x)  # 1D
    return x
