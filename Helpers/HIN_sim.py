...
from Helpers.util import cos_sim, list2dict
...
def cpt_p46_sim(dict1, dict2):
    ...
    if denom == 0:
        ...
        return sum(
            [len(dict1[key]) for key in dict1.keys() & dict2.keys()]) / denom  # 公用的service所包含的category/provider之和
...
class mashup_HIN_sims(object):
    ...
    #                 mashup_tags = self.gd.unpadded_encoded_mashup_tags
    #
    ...
    #             p2_sim = cpt_content_sim (self.ws, self.gd.unpadded_encoded_mashup_texts[min_m_id], self.gd.unpadded_encoded_mashup_texts[max_m_id])
    #         self_sim_dict[(min_m_id, max_m_id)] = p2_sim
    #     else:
    #         p2_sim = self_sim_dict[(min_m_id, max_m_id)]
    #     return p2_sim

    def get_p1_sim(self, min_m_id, max_m_id, mTagFalseSem, mashup_tags=None):
        """
    ...
    def get_p1_sim_sem(self, min_m_id, max_m_id, mTagTrueSem):
        """
        ...
        if (min_m_id, max_m_id) not in self.p1_sims_sem.keys():
            # print(min_m_id, max_m_id,'not in p1 sim!')
            self.flag1_sem = True
            p1_sim = cos_sim(self.mashup_tag_features[min_m_id],
                             self.mashup_tag_features[max_m_id])  # 利用外部传入的tag feature,应该和参数名一致
            self.p1_sims_sem[(min_m_id, max_m_id)] = p1_sim
        else:
            p1_sim = self.p1_sims_sem[(min_m_id, max_m_id)]
        return p1_sim

    def get_p2_sim(self, min_m_id, max_m_id, mTextFalseSem):
        """
        ...
        if (min_m_id, max_m_id) not in self.p2_sims.keys():
            ...
            self.flag2 = True
            # 1.编码过的文本索引也存储起来 2.不用每次编码，不需要加0，增加工作量...
            p2_sim = cpt_content_sim(self.ws, self.gd.unpadded_encoded_mashup_texts[min_m_id],
                                     self.gd.unpadded_encoded_mashup_texts[max_m_id])
    ...
    def get_p3_sim(self, m_id2, mashup_slt_apis_list):
        ...
        if _key not in self.p3_sims.keys():
            self.flag3 = True
            p3_sim = cpt_p13_sim(mashup_slt_apis_list, self.mashup_apis_dict[m_id2])
            self.p3_sims[_key] = p3_sim
        else:
            p3_sim = self.p3_sims[_key]
        return p3_sim

    def get_p4_sim(self, m_id2, mashup_slt_apis_list, aTagSem, api_tags=None):
        ...
        if aTagSem == 'MetaPath':
            ...
            sim_dict = self.p4_sims
        else:  # 其他形式的tag特征
            if self.p4_sims_sem is None:
                self.p4_sims_sem = self.load_sims('p4_sims_sem{}.dat'.format(aTagSem))
            sim_dict = self.p4_sims_sem

        _key = (m_id2, tuple(mashup_slt_apis_list))
        if _key not in sim_dict.keys():
            if aTagSem == 'MetaPath':
                self.flag4 = True
                if api_tags is None:  # 用户不传入则使用默认
                    api_tags = self.gd.unpadded_encoded_api_tags

                m1_api_category = {a_id: set(api_tags[a_id]) for a_id in mashup_slt_apis_list}
                m2_api_category = {a_id: set(api_tags[a_id]) for a_id in self.mashup_apis_dict[m_id2]}
                ...
                tag_sim_matrix = [[cos_sim(self.api_tag_features[a_id1], self.api_tag_features[a_id2])
                                   for a_id1 in mashup_slt_apis_list] for a_id2 in self.mashup_apis_dict[m_id2]]
                p4_sim = cpt_2list_sim(tag_sim_matrix)
            sim_dict[_key] = p4_sim
        else:
            p4_sim = sim_dict[_key]
        return p4_sim

    def get_p5_sim(self, m_id2, mashup_slt_apis_list, aTextSem='EmbMax'):
        # p5: SBS-service-content-service-SBS
        # 比较services的拼接文本相似度: IsRec使用的是最大化集合的做法!

        if self.p5_sims is None:
            self.p5_sims = self.load_sims('p5_sims{}.dat'.format(aTextSem))
        sim_dict = self.p5_sims

        _key = (m_id2, tuple(mashup_slt_apis_list))
        if _key not in sim_dict.keys():
            self.flag5 = True
            m2_apis = self.mashup_apis_dict[m_id2]

            m1_api_texts, m2_api_texts = [], []
            for api_id in mashup_slt_apis_list:
                m1_api_texts.extend(self.gd.unpadded_encoded_api_texts[api_id])  # 未padding
            for api_id in m2_apis:
                m2_api_texts.extend(self.gd.unpadded_encoded_api_texts[api_id])
            if  aTextSem=='EmbMax':
                p5_sim = cpt_content_sim(self.ws, m1_api_texts, m2_api_texts)
            else:
                raise ValueError('WRONG aTextSem mode for p5_sims!') # TODO
            sim_dict[_key] = p5_sim
        else:
            p5_sim = sim_dict[_key]
        return p5_sim
    ...
    def get_p6_sim(self, m_id2, mashup_slt_apis_list):
        ...
        if _key not in self.p6_sims.keys():
            ...
            m2_api_provider = {a_id: set(self.api_id2provider[a_id]) for a_id in self.mashup_apis_dict[m_id2]}
    ...
    # 新的得到sim的方法
    def new_get_mashup_HIN_sims(self, m_id1, m_id2, mTagFalseSem=None, mashup_tags=None, mTagTrueSem=None,
                                mTextFalseSem=None, mTextTrueSem=None):
        min_m_id = min(m_id1, m_id2)
        max_m_id = max(m_id1, m_id2)
    ...
    def save_changes(self):
        ...
        for i in range(len(sem_sims_list)):
            self.save_sim(flags_sem[i], 'p{}_sims_sem{}.dat'.format(sem_sims_index_list[i], self.semantic_name),
                          sem_sims_list[i])

    def save_changes(self, mTagFalseSem=None, mashup_tags=None, mTagTrueSem=None, mTextFalseSem=None,
                     mTextTrueSem=None):
        """
        新的调用之后，保存计算过的sim
        :return:
