...
from core.dataset import meta_data
from text_utils.word_embedding import get_word_embedding
from Helpers.util import cos_sim

class cpt_DHSR_Sim(object):
    def __init__(self, data_dir, embedding_name, embedding_dim, tag_coefficient, k, b, weighted_intervals,
        ...
        self.wordindex2IDF = {}
        self.average_len = 0
        self.stopwords= data_repository.get_args().stop_words if remove_stopwords else set()
    ...
    def get_data(self):
        ...
        if isinstance(api_descriptions[0], str):
            ...
            self.mashup_descriptions = [[self.word2inedx.get(word) for word in text] for text in  mashup_descriptions]
            self.api_descriptions = [[self.word2inedx.get(word) for word in text] for text in api_descriptions]
        ...
        embedding = get_word_embedding(self.embedding_name, self.embedding_dim)
...
def get_sims_dict(process_new,remove_stopwords):
    ...
    cs = cpt_DHSR_Sim(data_repository.get_args().cur_data_dir, embedding_name, embedding_dim, tag_coefficient, k, b, weighted_intervals,
                      unweighted_intervals, process_new, remove_stopwords)
    return cs # 返回对象
