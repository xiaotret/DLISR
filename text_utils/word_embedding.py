...
def get_word_embedding(embedding_obj, embedding_name, word,dimension,initize='random'):
    """
    得到一个词的embedding向量，对不存在的词，可以选择0初始化，随机初始化，或者按-切割再相加
    :param embedding_obj: 对象
    ...
    if embedding_name == 'glove':
        vector = embedding_obj.get(word)
    elif embedding_name == 'google_news':
        vector = embedding_obj.get_vector(word)
...
def get_embedding_obj(embedding_name, dimension=50):
    """
    读取预训练的embedding模型
    :param embedding_name:
    :param dimension:
    ...
    if embedding_name == 'google_news':
        embedding_obj = gensim.models.KeyedVectors.load_word2vec_format(glove_embedding_path, binary=True)
    elif embedding_name == 'glove':
        embedding_obj = {}
        with open(os.path.join(google_embedding_path,
                               "glove.6B.{}d.txt".format(dimension)), encoding='utf-8') as f:  # dict: word->embedding(array)
            for line in f:
                values = line.split()
                word = values[0]
                embedding_obj[word] = np.asarray(values[1:], dtype='float32')
    print('Found %s word vectors in pre_trained embedding.' % len(embedding_obj))
...
def get_embedding_matrix(word2index, embedding_name, dimension=50):
    ...
    :return: 2D array
    """
    embedding_obj=get_embedding_obj(embedding_name, dimension)
    ...
    for word, index in word2index.items(): # keras 文本预处理后得到的字典 按词频 对单词编码
        embedding_matrix[index]=get_word_embedding(embedding_obj, embedding_name, word, dimension)
