...
from keras.layers import Dense, Input, Concatenate, Embedding, Multiply
from keras.models import Model
from core.processing_data import get_mashup_api_allCategories
...
class gx_text_tag_continue_model (gx_text_tag_model):
    ...
    def set_attribute(self): # ,all_mashup_num,all_api_num,his_m_ids
        ...
        self.his_m_ids = data_repository.get_ds().his_mashup_ids
    ...
    def get_model(self): # 'old' 'new'
        if self.model is None:
            ...
            for layer in self.model.layers:
                print(layer.name)
            print ('built whole whole_model, done!')
        ...
        # plot_model (self.whole_model, to_file='text_tag_continue.png', show_shapes=True)
        return self.model
...
class gx_text_tag_continue_only_MLP_model (gx_text_tag_continue_model):
    ...
    def __init__(self,new_old,if_tag=True,if_tag_sim=True):
        ...
        self.implict_feat_dim = new_Para.param.implict_feat_dim
        # 加入一个padding用的虚拟的api的映射
        self.i_factors_matrix = np.zeros((meta_data.api_num+1,self.implict_feat_dim)) # 待改！！！传参和参数类，数据类之间的平衡
    ...
    def set_embedding_weights(self,recommend_model):
        ...
        print('after training the text_tag whole_model, the embedding of padding 0:')
    ...
    def build_m_i_matrix(self):
        ...
        for index in range(len(data_repository.get_ds().train_mashup_id_list)):
            if  data_repository.get_ds().train_labels[index]==1:
                m_index=self.m_id2index[data_repository.get_ds().train_mashup_id_list[index]]
                self.m_a_matrix[m_index][data_repository.get_ds().train_api_id_list[index]]=1
    ...
    def get_instances_old(self,mashup_id_list, api_id_list,mashup_slt_apis_list=None):
        ...
        # with open(os.path.join(data_repository.get_ds().data_root, 'final_{}sims_mashup_sims_instances.sim'.format(self.total_sim_dim)), 'ab+') as f:
        #     pickle.dump(mashup_sims_instances, f)
    ...
    def get_model(self,_model):
        ...
        if self.model is None:
            ...
            if self.if_implict:
                if self.NI_handle_slt_apis_mode:
                    ...
                    if self.NI_handle_slt_apis_mode=='attention':
                        ...
                        slt_api_implict_embeddings = Reshape((self.implict_feat_dim * new_Para.param.slt_item_num))(slt_api_implict_embeddings)  # (None,75)
            ...
            if not self.no_CI:
                ...
                self.whole_model.get_layer('cf1').set_weights(w_dense2)
                self.whole_model.get_layer('cf2').set_weights(w_dense3)
                self.whole_model.get_layer('cf3').set_weights(w_dense4)
            """
        return self.model
    ...
    def save_sim_weight(self):
        ...
        root = data_repository.get_ds().model_path.format(self.get_simple_name())
