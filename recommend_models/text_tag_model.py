...
from keras.layers import Lambda, Concatenate, Embedding, Multiply
...
class gx_text_tag_model (gx_model):
    ...
    def show_mashup_text_tag_embeddings(self,train_data,mashup_num=10):
        ...
        """
        """
        text_tag_middle_model = Model(inputs=[self.whole_model.inputs[0], self.whole_model.inputs[2]],
        ...
        # 两个看embedding结果的文件
        text_embeddings_path = os.path.join(data_repository.get_ds().model_path,'mashup_text_embeddings.dat')
    ...
    def get_model(self):
        if self.model==None:
            ...
            """
            for layer in self.whole_model.layers:
                print(layer.name)
    ...
    def get_mashup_text_tag_features(self,mashup_ids,save_results=True):
        ...
        if save_results:
            text_features_path = os.path.join(data_repository.get_ds().model_path.format(self.get_simple_name()), 'mashup_text_features.dat')
            tag_features_path = os.path.join(data_repository.get_ds().model_path.format(self.get_simple_name()),
...
class gx_text_tag_only_MLP_model (gx_text_tag_model):
    def __init__(self, mf_fc_unit_nums,
                 u_factors_matrix, i_factors_matrix, m_id2index,i_id2index, ini_features_array, implict_feat_dim, topK,CF_self_1st_merge,cf_unit_nums,text_weight=0.5,
        ...
        self.implict_feat_dim = implict_feat_dim
    ...
    def initialize(self, text_tag_recommend_model, text_tag_model, train_mashup_id_list, train_api_id_list,
        ...
        for i in range (len (all_feature_mashup_ids)):  # 为所有mashup找最近
            ...
            cf_feature = np.zeros ((self.implict_feat_dim))
    ...
    def get_model(self):
        ...
        mashup_cf = Input (shape=(self.implict_feat_dim,), dtype='float32')
        api_cf = Input (shape=(self.implict_feat_dim,), dtype='float32')
    ...
    def get_instances(self, mashup_id_list, api_id_list):
        ...
        api_zeros = np.zeros ((self.implict_feat_dim))
...
class gx_text_tag_only_MLP_model_new (gx_text_tag_model):
    ...
    def __init__(self):
        ...
        self.i_factors_matrix = np.zeros((meta_data.api_num,self.implict_feat_dim)) # 待改！！！传参和参数类，数据类之间的平衡
