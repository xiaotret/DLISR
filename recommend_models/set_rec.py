...
from text_utils.encoding_padding_texts import get_default_encoding_texts
...
from core.evalute import summary
...
class set_rec (object):
    def __init__(self, cluster_method,embedding_ts=pow(10,-5),cluster_ts=pow(10,-4), cluster_num=10,embedding_mode='W'):
        ...
        self.path = os.path.join (data_repository.get_ds().root_path, 'set_rec')  # 存放该模型的数据  0/set_rec
    ...
    def get_train_instances(self):
        ...
        num_train_samples = len (data_repository.get_ds().train_mashup_id_list)
        train_m_ids = set ()
        inters = []  # 存储各个不同m-id的分界index
        for index in range (num_train_samples):
            temp_m_id = data_repository.get_ds().train_mashup_id_list[index]
        ...
        for i in range (len (inters) - 1):
            ...
            for index in range (end_index - start_index):
                if data_repository.get_ds().train_labels[index] == 1:
                    ...
                    self.train_instances.append ((data_repository.get_ds().train_mashup_id_list[pos_index],
                                                  data_repository.get_ds().train_api_id_list[pos_index], data_repository.get_ds().train_api_id_list[neg_index]))
                else:
                    break
    ...
    def transfer_2_local(self):
        m_ids, a_ids = zip (*data_repository.get_ds().train_mashup_api_list)
        ...
        for m_id, a_id in data_repository.get_ds().train_mashup_api_list:
    ...
    def recommend(self, topKs=[5]):
        ...
        for i in range (test_m_num):
            ...
            test_m_id = data_repository.get_ds().test_mashup_id_list[i][0]  # 每个mashup id
            ...
            for topK in topKs:  # NDCG@5 or NDCG@10
                ...
                # 调用evalute函数
                a_mashup_indicators.append(list (evaluate (max_k_candidates, data_repository.get_ds().grounds[i], topK)))  # 评价得到五个指标，K对NDCG等有用
