...
class top_MLP(recommend_Model):
    def __init__(self, args, CI_recommend_model=None, CI_model=None, CI_model_dir=None, NI_recommend_model1=None,
                 NI_model1=None, NI_recommend_model2=None, NI_model2=None):
        super(top_MLP, self).__init__(args)
        # NI_recommend_model用于隐式交互，NI_recommend_model2用于显式交互
        ...
        self.args = args
        self.CI_dim = self.args.content_fc_unit_nums[-1]
        self.NI_dim1 = self.args.imp_fc_unit_nums[-1]
        self.NI_dim2 = self.args.cor_fc_unit_nums[-1] if self.args.data_mode == 'newScene' else \
        self.args.exp_fc_unit_nums[-1]

        self.model = None
        self.CI_recommend_model = CI_recommend_model
        self.NI_recommend_model1 = NI_recommend_model1
        self.NI_recommend_model2 = NI_recommend_model2
        self.CI_model = CI_model
        self.NI_model1 = NI_model1
        self.NI_model2 = NI_model2

        if self.CI_recommend_model is not None and self.CI_model is not None:
            self.CI_midMLP_feature_obj = get_midMLP_feature(self.CI_recommend_model, self.CI_model)
        else:
            self.CI_midMLP_feature_obj = None

        if self.NI_recommend_model1 is not None and self.NI_model1 is not None:
            self.NI_midMLP_feature_obj1 = get_midMLP_feature(self.NI_recommend_model1, self.NI_model1)
        else:
            self.NI_midMLP_feature_obj1 = None

        if self.NI_recommend_model2 is not None and self.NI_model2 is not None:
            self.NI_midMLP_feature_obj2 = get_midMLP_feature(self.NI_recommend_model2, self.NI_model2, if_explict=True)
        else:
            self.NI_midMLP_feature_obj2 = None

        self.optimizer = Adam(lr=self.args.topMLP_learning_rate)
    ...
    def get_simple_name(self):
        if not self.simple_name:
            ...
            NI_name1 = 'IMP' if self.NI_recommend_model1 is not None else ''
            if self.NI_recommend_model2 is None:
                NI_name2 = ''
            else:
                if self.args.data_mode == 'new':
                    NI_name2 = '_COR'
                elif self.args.data_mode == 'old':
                    NI_name2 = '_EXP'
            self.simple_name = '{}_{}_{}_{}_{}{}_MLP{}{}top{}'.format(CI_name, self.args.simple_CI_slt_mode,
                                                                          NI_name1,self.args.simple_NI_slt_mode,self.args.NI_sim_mode,
                                                                          NI_name2,
                                                                          self.args.predict_fc_unit_nums, self.args.topMLP_learning_rate,
                                                                          self.args.topK).replace(',', '_')
        return self.simple_name

    def set_paths(self):
        # 路径在CI下面，依次是CI/top_MLP
        if self.CI_recommend_model is not None:
            self.CI_model_dir = self.CI_recommend_model.model_dir
    ...
    def get_model(self):
        if self.model is None:
            ...
            NI_input1 = Input(shape=(self.NI_dim1,), dtype='float32', name='implict_NI_input')
            NI_input2 = Input(shape=(self.NI_dim2,), dtype='float32', name='explict_NI_input')
            if self.CI_model is None:  # INI+ENI
                inputs = [NI_input1, NI_input2]
            else:
                if self.NI_model1 is None and self.NI_model2 is not None:
                    inputs = [CI_input, NI_input2]
                if self.NI_model2 is None and self.NI_model1 is not None:
                    inputs = [CI_input, NI_input1]
                if self.NI_model1 is not None and self.NI_model2 is not None:
                    inputs = [CI_input, NI_input1, NI_input2]
            predict_vector = Concatenate()(inputs)

            for index, unit_num in enumerate(self.args.predict_fc_unit_nums):
                predict_vector = Dense(unit_num, name='predict_dense_{}'.format(index), activation='relu',
                                       kernel_regularizer=l2(self.args.l2_reg))(predict_vector)
            ...
            if self.args.final_activation == 'softmax':
                ...
                predict_result = Dense(1, activation='sigmoid', kernel_initializer='lecun_uniform', name="prediction")(
                    predict_vector)
            print('build top_MLP whole_model,done!')
    ...
    def get_instances(self,data, pairwise_train_phase_flag=True):
        ...
        if self.CI_model is not None:
            CI_midMLP_features = np.array(self.CI_midMLP_feature_obj.get_midMLP_feature('text_tag_feature_extracter',data,
                                                                                        pairwise_train_phase_flag=pairwise_train_phase_flag))

        if self.NI_model1 is not None:
            NI_layer_name = 'implict_dense_{}'.format(len(self.args.imp_fc_unit_nums))  # 中间层名字
            NI_midMLP_features = np.array(self.NI_midMLP_feature_obj1.get_midMLP_feature(NI_layer_name, data,
                                                                                         pairwise_train_phase_flag=pairwise_train_phase_flag))
        if self.NI_model2 is not None:
            name = 'explict' if self.args.data_mode == 'old' else 'cor' # TODO
            NI_layer_name2 = '{}_dense_{}'.format(name, len(self.args.exp_fc_unit_nums))  # 中间层名字
            NI_midMLP_features2 = np.array(self.NI_midMLP_feature_obj2.get_midMLP_feature(NI_layer_name2, data))
        # 中间结果作为输入
        instances = dict()
        if self.CI_model is None:  # INI+ENI
            instances = {'implict_NI_input':NI_midMLP_features,'explict_NI_input':NI_midMLP_features2}
        else:
            instances['CI_input'] = CI_midMLP_features
            if self.NI_model1 is not None:
                instances['implict_NI_input'] = NI_midMLP_features
            if self.NI_model2 is not None:
                instances['explict_NI_input'] = NI_midMLP_features2
        return instances

    def save_sth(self):
        if self.CI_midMLP_feature_obj is not None:
            self.CI_midMLP_feature_obj.save_sth()
        if self.NI_midMLP_feature_obj1 is not None:
            self.NI_midMLP_feature_obj1.save_sth()
...
class fine_Tune(object):
    def __init__(self, CI_recommend_model, CI_model, NI_recommend_model, NI_model, top_MLP_recommend_model=None,
                 top_MLP_model=None, model_mode='ft', ft_mode='', lr=0.0001):
        """
        ...
        self.CI_recommend_model = CI_recommend_model
        self.NI_recommend_model1 = NI_recommend_model
        ...
        self.args.predict_fc_unit_nums = self.args.predict_fc_unit_nums

        self.ft_mode = ft_mode  # 默认只训练topMLP,可以三个MLP连同attention一起训练，还是连同特征提取器一起训练
        if self.model_mode == 'co_train':
            self.simple_name = 'co_train_CI_{}_NI_{}_{}'.format(self.args.simple_CI_slt_mode,
                                                                self.args.NI_OL_mode, self.lr)
    ...
    def get_model(self):
        if self.model is None:
            ...
            NI_layer_name = 'implict_dense_{}'.format(len(self.args.cf_unit_nums) - 1)  # 中间层名字
            ...
            print('build whole fine-tuning whole_model,done!')
            self.model = Model(inputs=[*self.CI_model.input, *self.NI_model.input], outputs=[predict_result],
                               name='whole_model')
    ...
    def pre_fine_tune(self):
        layer_num_below_top_MLP = 2 + len(self.args.predict_fc_unit_nums)  # MLP,dropout,prediction
        ...
        # top_MLP参数初始化
        for index in range(len(self.args.predict_fc_unit_nums)):
        ...
        def train_CINI():
            # CI中MLP设为可训练
            for index in range(len(self.args.content_fc_unit_nums[:-1])):
            ...
            if 'attention' in self.args.CI_handle_slt_apis_mode:
                ...
                # self.NI_model1.get_layer('implict_attBlock').trainable = True
        ...
        if self.ft_mode == 'topMLP':
            ...
            # for layer in self.whole_model.layers[:-1 * layer_num_below_top_MLP]:
            #     layer.trainable = True
    ...
    def get_instances(self, mashup_id_list, api_id_list, mashup_slt_apis_list=None, test_phase_flag=True):
        ...
        NI_examples = self.NI_recommend_model1.get_instances(mashup_id_list, api_id_list, mashup_slt_apis_list,
                                                             pairwise_train_phase_flag=test_phase_flag)
...
class PNCF_doubleTower(object):
    ...
    def __init__(self, CI_recommend_model=None, CI_model=None, CI_model_dir=None, NI_recommend_model=None,
                 NI_model=None, new_old='new', model_name='PNCF'):
        ...
        self.mashup_texts_features, self.mashup_tag_features, self.api_texts_features, self.api_tag_features = \
            self.CI_recommend_model.get_mashup_api_features(self.CI_recommend_model.all_mashup_num,
                                                            self.CI_recommend_model.all_api_num)
        ...
        self.lr = self.args.CI_learning_rate  # 暂时用CI的
        self.optimizer = Adam(lr=self.lr)

        self.simple_name = '{}_CI_{}_INI_{}_topMLP{}{}top{}'.format(self.model_name, self.args.simple_CI_slt_mode,
                                                                      self.args.NI_OL_mode,
                                                                      self.args.predict_fc_unit_nums, self.lr,
                                                                      self.args.topK).replace(',', '_')
    ...
    def get_model(self):
        if self.model is None:
            ...
            if self.args.final_activation == 'softmax':
                ...
                predict_result = Dense(1, activation='sigmoid', kernel_initializer='lecun_uniform', name="prediction")(
                    predict_vector)
            print('build PNCF whole_model,done!')
    ...
    def get_instances(self, mashup_id_list, api_id_list, mashup_slt_apis_list=None, test_phase_flag=True):
        ...
        mashup_NI_features = np.array(
            [self.NI_recommend_model1.mid_sltAids_2NI_feas[(mashup_id_list[i], tuple(mashup_slt_apis_list[i]))] for i in
             range(len(mashup_id_list))])
        api_NI_features = np.array([self.NI_recommend_model1.i_factors_matrix[a_id] for a_id in api_id_list])
...
class DINRec_model(object):
    def __init__(self, CI_recommend_model=None, CI_model=None, CI_model_dir=None, NI_recommend_model=None,
        ...
        self.simple_name = '{}_CI_{}_INI_{}_topMLP{}{}top{}_2'.format(self.model_name, self.args.simple_CI_slt_mode,
                                                                      self.args.NI_OL_mode,
                                                                      self.args.predict_fc_unit_nums, self.lr,
                                                                      self.args.topK).replace(',', '_')
        self.model_dir = os.path.join(CI_model_dir, self.simple_name)
    ...
    def get_model(self):
        if self.model is None:
            ...
            slt_api_num = self.args.slt_item_num
            slt_apis_input = Input(shape=(slt_api_num, self.full_dim,), dtype='float32',
                                   name='slt_apis_input')  # (3,125)
            ...
            if self.model_name == 'MLP_embedding':
                ...
                x = AveragePooling2D(pool_size=(self.args.slt_item_num, 1))(x)
            ...
            print('build DIN_rec whole_model,done!')
    ...
    def get_instances(self, mashup_id_list, api_id_list, mashup_slt_apis_list=None, test_phase_flag=True):
        ...
        mashup_slt_apis_array = np.ones((len(mashup_id_list), self.args.slt_item_num),
                                        dtype='int32') * self.CI_recommend_model.all_api_num  # default setting,api_num
        ...
        slt_apis_features = np.array([[np.hstack(
            (self.api_CI_features[slt_api], self.NI_recommend_model1.i_factors_matrix[slt_api])) for slt_api in slt_apis]
                                      for slt_apis in mashup_slt_apis_array])
