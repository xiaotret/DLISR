...
class gx_text_only_model(gx_model):
    ...
    def get_mashup_api_text_features(self, mashup_ids = None, api_ids = None, save_results=True):
        ...
        if save_results:
            mashup_text_features_path = os.path.join(data_repository.get_ds().model_path.format(self.get_simple_name()), 'mashup_text_features.dat')
            api_text_features_path = os.path.join(data_repository.get_ds().model_path.format(self.get_simple_name()),'api_test_features.dat')
...
class gx_text_only_MLP_model(gx_text_only_model):
    def __init__(self,sim_mode):
        ...
        self.implict_feat_dim = new_Para.param.implict_feat_dim # cf部分维度
        self.text_feature_dim = new_Para.param.inception_fc_unit_nums[-1]
        # i_factors_matrix 按照全局id排序; UI矩阵也是按照api id排序
        self.a_cf_features = np.zeros((meta_data.api_num,self.implict_feat_dim))
        ...
        self.his_mashup_ids=data_repository.get_ds().his_mashup_ids
    ...
    def get_model(self,_model):
        ...
        if self.model is None:
            ...
            if self.if_implict: # 隐式历史交互
                mashup_cf_features = Input(shape=(self.implict_feat_dim,), dtype='float32')
                api_cf_features = Input(shape=(self.implict_feat_dim,), dtype='float32')
            ...
            if not self.if_implict and not self.if_explict:
                print('please use text_only whole_model!')
                sys.exit()
