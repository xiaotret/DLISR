...
from core.new_para_setting import new_Para, new_Para_
from core.split_data import split_dataset_for_newScene_KCV, split_dataset_for_oldScene_KCV
from core.run_models import test_simModes, bl_PasRec, DINRec, bl_DHSR_new, baselines, bl_IsRec_best, CI_NI_fineTuning
...
def tst_kcv_paraObj(old_new = 'new', if_few=False): # 新模型还是旧模型
    ...
    def get_dataset_generator():
        if new_Para.param.data_mode == 'newScene':
            dataset_generator = split_dataset_for_newScene_KCV(*new_Para.param.split_newScene_dataset_settings)
