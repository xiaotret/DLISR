...
from core.evalute import evalute_by_epoch, analyze_result
# from run_deepCTR.run_MISR_deepFM import run_deepFM
# from recommend_models.NI_Model_online_new import NI_Model
...
from core.dataset import dataset, meta_data
from core.split_data import split_dataset_for_newScene_KCV, split_dataset_for_oldScene_KCV
...
def bl_PNCF(a_dataset):
    ...
    for mlp_mode in mlp_modes:
        for concate_mode in concate_modes:
            ...
            pncf_model = train_model(pncf_rec_model, pncf_model, a_dataset.train_data,
...
def text_only_oldScene(a_dataset):
    ...
    text_model = train_model(text_recommend_model, text_model, a_dataset.train_data, a_dataset.test_data,
    ...
    train_model(text_only_MLP_recommend_model, text_only_MLP_model, a_dataset.train_data, a_dataset.test_data,
    ...
    train_model(text_tag_MLP_only_continue_recommend_model, text_tag_MLP_only_continue_model,
...
def bl_DHSR(a_dataset):
    ...
    dhsr_model = train_model(dhsr_recommend_model, dhsr_model, train_data, test_data,*new_Para.param.train_paras)  # 'monitor loss&acc'
...
def bl_IsRec_best(a_dataset):
    ...
    for cluster_mode_topic_num in cluster_mode_topic_nums:
        HINRec_model = HINRec(model_name=model_name, semantic_mode='TF_IDF', epoch_num=epoch_num,
...
def text_tag():
    ...
    # 训练测试
    text_tag_model = train_model(text_tag_recommend_model, text_tag_model, train_data, test_data,
                                           *new_Para.param.train_paras)  # 'monitor loss&acc'
    ...
    if if_whole:
        ...
        train_model(text_tag_MLP_only_continue_recommend_model, text_tag_MLP_only_continue_model, train_data,
                              test_data, *new_Para.param.train_paras)
        """
        # 新场景下针对每种个数的进行测试
        testsets = data_repository.get_ds().split_test_ins_by_sltNum()
...
def CI_NI_fineTuning():
    ...
    # NI_model_obj = train_model(NI_recommend_model, NI_model_obj,train_data, test_data, *new_Para.param.train_paras)
    ...
    #                                   NI_model1=implict_NI_model_obj,
    ...
    # fineTune_model = train_model(fineTune_recommend_model, fineTune_model, train_data, test_data,*new_Para.param.train_paras)
...
def NI_online():  # 可以用于CI,NI,topMLP,ft等
    ...
    # # data_repository.get_ds().UV_obj.save_onlineNode2vec()
    ...
    #
    # # # CI+ implict
    # top_MLP_recommend_model = top_MLP(CI_recommend_model,CI_model_obj,NI_recommend_model=NI_OL_recommend_model,NI_model1=NI_OL_model_obj)
    # top_MLP_model = top_MLP_recommend_model.get_model()
    # top_MLP_model = train_model(top_MLP_recommend_model, top_MLP_model, train_data, test_data,*new_Para.param.train_paras)
    # top_MLP_recommend_model.save_sth() # 存储训练测试过程中使用的所有实例的中间结果
    #
...
def get_preTrain_CINI_model():
    ...
    CI_recommend_model.prepare()
    CI_model_obj = CI_recommend_model.get_model()
    CI_model_obj = train_model(CI_recommend_model, CI_model_obj, train_data, test_data,
...
def co_trainCINI():  # 参数完全随机化，联合训练CI和NI
    ...
    co_trainCINI_model = train_model(co_trainCINI_recommend_model, co_trainCINI_model, train_data, test_data,
                                               *new_Para.param.train_paras)
...
def test_PNCF_doubleTower_OR_DIN():
    ...
    DINRec_model_obj = train_model(DINRec_recommend_model, DINRec_model_obj, train_data, test_data,
                                             *new_Para.param.train_paras)
...
def DINRec(a_dataset,new_old = 'new'):
    ...
    DINRec_model_obj = train_model(DINRec_model, DINRec_model_obj,train_data,test_data,*new_Para.param.train_paras)  # ,true_candidates_dict=HINRec_model.get_true_candi_apis() 'monitor loss&acc'
...
def test_simModes(a_dataset,new_old = 'new',if_few = False):
    ...
    HINRec_model = HINRec(model_name=new_Para.param.NI_OL_mode, epoch_num=20, neighbor_size=15, topTopicNum=3)
    ...
    # #
    # # # # # CI+ implict
    top_MLP_recommend_model = top_MLP(CI_recommend_model, CI_model_obj, NI_recommend_model1=NI_OL_recommend_model, NI_model1=NI_OL_model_obj)
    top_MLP_model = top_MLP_recommend_model.get_model()
    top_MLP_model = train_model(top_MLP_recommend_model, top_MLP_model, train_data, test_data,*new_Para.param.train_paras)
    top_MLP_recommend_model.save_sth() # 存储训练测试过程中使用的所有实例的中间结果
